...
public:
    ...
    void mouseDown (const MouseEvent& e)
    {
        ...
        if (e.mods.isPopupMenu())
        {
            ...
            {
                int newDestNodeId = 0;
                int newSourceNodeId = 0;
                for (int i = graph.getNumConnections(); --i >= 0;){
                    const AudioProcessorGraph::Connection *c = graph.getConnection(i);
                    if (c->sourceNodeId == filterID)
                    {
                        newDestNodeId = c->destNodeId;
                        graph.removeConnection(i);
                    }else if(c->destNodeId == filterID)
                    {
                        newSourceNodeId = c->sourceNodeId;
                        graph.removeConnection(i);
                    }
                }
                if(newSourceNodeId != 0 || newDestNodeId != 0){
                    for(int i=0; i < 2; i++)
                        graph.addConnection(newSourceNodeId,i,newDestNodeId,i);
                }
    ...
    void resized()
    {
        int cnumIns = midiIn ? numIns-1 : numIns;
        int cnumOut = midiOut ? numOuts-1 : numOuts;

        for (int i = 0; i < getNumChildComponents(); ++i)
        {
            if (PinComponent* const pc = dynamic_cast <PinComponent*> (getChildComponent(i)))
            {
                const int total = pc->isInput ? cnumIns : cnumOut;
                const int index = pc->index == FilterGraph::midiChannelNumber ? (total - 1) : pc->index;

                if(pc->index == FilterGraph::midiChannelNumber)
                {
                pc->setBounds (proportionOfWidth (0.75f) - pinSize / 2,
                ...
                float x;
                if(midiIn || midiOut)
                    x = proportionOfWidth ((1 + index) / (total + 1.0f))*0.5f - 0.25f - pinSize / 2;
                else
                    x = proportionOfWidth ((1 + index) / (total + 1.0f)) - pinSize / 2;
    ...
    void update()
    {
        ...
        w = jmax (w, 16 + jmin (textWidth, 600));
        if (textWidth > 600)
            h = 100;
...

        {
            ...
            if(midiIn || midiOut)
                ...
                setCentreRelative ((float) x-0.25f+0.125f, (float) y);
        }
...
void GraphEditorPanel::createNewPlugin (const PluginDescription* desc, int x, int y)
{
    ...
    uint32 audioInID = 1;
    uint32 audioOutID = 3;
	uint32 numNodes = graph.getNumFilters()-2;
	uint32 nodeID = graph.addFilter(desc, 0.75f, 0.05f + numNodes*0.075f);
    const AudioProcessorGraph::Node::Ptr f (graph.getNode (graph.getNumFilters()-1));

    if(graph.getNumConnections() == 0){
        for(int i = 0; i<2; i++){
        graph.addConnection(audioInID,i,nodeID,i);
        graph.addConnection(nodeID,i,audioOutID,i);
        }
    }
    else{
        for (int i = graph.getNumConnections(); --i >= 0;){
            const AudioProcessorGraph::Connection *connection = graph.getConnection(i);
            uint32 sourceNodeId0 = connection->sourceNodeId;
            int sourceChannelIndex0 = connection->sourceChannelIndex;

            if(connection->destNodeId == audioOutID){
                graph.removeConnection(i);
                graph.addConnection(nodeID,sourceChannelIndex0,audioOutID,sourceChannelIndex0);
                //connection->destNodeId = nodeID;
                graph.addConnection(sourceNodeId0,sourceChannelIndex0,nodeID,sourceChannelIndex0);
            }
        }
       // graph.removeConnection(3,0);
