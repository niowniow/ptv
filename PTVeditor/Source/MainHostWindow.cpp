...
void MainHostWindow::initializeKPL()
{
  File oldwdir = File::getCurrentWorkingDirectory();
	...
	File dir = File(vstpath);
    FileSearchPath fsp;
	fsp.add(dir);
	StringArray pluginNames;
	pluginNames = format.searchPathsForPlugins(fsp, true);
	if (pluginNames.size() == 0){
		//std::cout << "Error: No Plugins found at " << fsp.toString() << std::endl;
		return;
	}
	formatManager.addDefaultFormats();
	OwnedArray <PluginDescription>  desc;
	knownPluginList.scanAndAddDragAndDroppedFiles(formatManager, pluginNames, desc);
    oldwdir.setAsCurrentWorkingDirectory();
    knownPluginList.addChangeListener (this);
