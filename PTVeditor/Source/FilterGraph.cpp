      ...
      formatManager (formatManager_), lastUID (0)
{
    ...
    addFilter (internalFormat.getDescriptionFor (InternalPluginFormat::audioInputFilter),  0.75f,  0.05f);
    addFilter (internalFormat.getDescriptionFor (InternalPluginFormat::midiInputFilter),   0.25f, 0.05f);
    addFilter (internalFormat.getDescriptionFor (InternalPluginFormat::audioOutputFilter), 0.75f,  0.95f);
...
XmlElement* createXmlPTV(PluginDescription pd)
{
	// implements the PluginDescription functions createXml but uses relative paths 
	// and extensions based on the platform

	XmlElement* const e = new XmlElement("PLUGIN");
	e->setAttribute("name", pd.name);
	if (pd.descriptiveName != pd.name)
		e->setAttribute("descriptiveName", pd.descriptiveName);

	e->setAttribute("format", pd.pluginFormatName);
	e->setAttribute("category", pd.category);
	e->setAttribute("manufacturer", pd.manufacturerName);
	e->setAttribute("version", pd.version);

	pd.fileOrIdentifier = pd.name;
	e->setAttribute("file", pd.fileOrIdentifier);
	e->setAttribute("uid", String::toHexString(pd.uid));
	e->setAttribute("isInstrument", pd.isInstrument);
	e->setAttribute("fileTime", String::toHexString(pd.lastFileModTime.toMilliseconds()));
	e->setAttribute("numInputs", pd.numInputChannels);
	e->setAttribute("numOutputs", pd.numOutputChannels);
	e->setAttribute("isShell", pd.hasSharedContainer);
